...
export const ProviderHomeScreen = ({ providerData, onLogout }) => {
  ...
  const fetchHelpRequests = async () => {
    try {
      ...
      setHelpRequests(requestsWithUsers);
    } catch (err) {
    ...
    >
      <View style={styles.cardContent}>
        ...
        <View style={styles.cardText}>
          <Text style={styles.userName}>{user?.nom_prenom || 'Nom non disponible'}</Text>
          <Text style={styles.requestDescription}>
            {request.type} - {request.description || 'Pas de description'}
          </Text>
